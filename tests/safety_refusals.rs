...
#[test]
fn handoff_json_refuses_missing_required_target_claim() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn handoff_json_refuses_conflicting_required_target_claim() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn assert_json_refuses_missing_required_claim() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
    ...
    let output = zaphod(
        dir.path(),
        ["assert", "--json", "--agent", "codex", "--require-claim"],
    );
...
#[test]
fn assert_json_refuses_conflicting_claim_for_agent() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
    ...
    let output = zaphod(dir.path(), ["assert", "--json", "--agent", "codex"]);
    ...
    assert_eq!(report["claim"]["requested_agent"], "codex");
    assert_eq!(report["claim"]["claim_allowed"], false);
    assert_eq!(report["claim"]["claim_required"], false);
    assert_eq!(report["claim"]["claim_owned"], false);
    assert_eq!(report["claim"]["conflict"]["agent"], "other");
    assert_eq!(report["claim"]["conflict"]["branch"], "feature/api");
    assert_eq!(report["failures"], json!([]));
    assert_stderr_contains(
        &output,
        "pair 'default' on branch 'feature/api' is already claimed by agent 'other'",
    );
}

#[test]
fn assert_json_refuses_missing_required_target_claim() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn assert_json_prioritizes_missing_current_claim_over_missing_target_claim() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn assert_json_refuses_conflicting_required_target_claim() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn claim_target_json_reports_conflicting_agent_without_switching() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn claim_json_reports_stale_conflicting_agent_without_takeover() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
"#,
    ...
    let output = zaphod(
        dir.path(),
        ["claim", "--json", "--agent", "other", "--stale-after", "1d"],
    );
...
#[test]
fn claim_json_reports_fresh_conflicting_agent() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
...
#[test]
fn claim_rejects_invalid_stale_claim_window() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());

    let output = zaphod(
        dir.path(),
        ["claim", "--agent", "codex", "--stale-after", "later"],
    );
...
#[test]
fn prune_claims_target_reports_current_branch_outside_pair() {
    let dir = TestDir::new("zaphod-cli-safety");
    init_repo_with_pair_branches(dir.path());
    let pair = zaphod(dir.path(), ["pair", "feature/api", "feature/ui"]);
    assert_success(&pair);
    ...
    let output = zaphod(
        dir.path(),
        ["prune-claims", "--target", "--stale-after", "1d"],
    );
